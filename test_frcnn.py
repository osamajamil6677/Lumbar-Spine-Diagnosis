...
wght_loc_list = ['./Weights/SPD/model_frcnn.hdf5','./Weights/SS/model_frcnn.hdf5','./Weights/EPD/model_frcnn.hdf5','./Weights/ADB/model_frcnn.hdf5']
for w_path, sv_loc in zip(wght_loc_list,sve_loc_list):
	...
	#print(bbx_df.head())
	#train = pd.read_csv(r'C:\Users\Hamza\Desktop\FYP2\SPD\keras-frcnn\bbx_df.csv')
#print(disease_bbx_list[0])	
...
# img_name_list_ch = []
# for idx_ch, img_name_ch in enumerate(sorted(os.listdir(img_path))):
# 	if not img_name_ch.lower().endswith(('.bmp', '.jpeg', '.jpg', '.png', '.tif', '.tiff')):
# 		continue

# 	img_name_list_ch.append(img_name_ch)

# 	filepath_ch = os.path.join(img_path,img_name_ch)

# 	img_ch = cv2.imread(filepath_ch)
